...
import { X, Bell, BookOpen, Check, ChevronDown, MapPin } from 'lucide-react';
import { CLASSES } from '@/data/scheduleData';

interface Props {
  username: string;
  remindMins: number;
  onSave: (username: string, remindMins: number) => void;
  onClose: () => void;
}
...
// ── Settings Modal ────────────────────────────────────────────────────────────
export default function SettingsModal({ username: initUsername, remindMins: initMins, onSave, onClose }: Props) {
  ...
  const handleSave = () => {
    const name = username.trim() || '同学';
    localStorage.setItem('bb-username',    name);
    localStorage.setItem('bb-remind-mins', String(remindMins));
    onSave(name, remindMins);
    onClose();
  };
    ...
    >
      <div className="w-full max-w-sm bg-background rounded-t-3xl overflow-y-auto max-h-[88vh] shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-border shrink-0">
          <h2 className="text-lg font-black">偏好设置</h2>
          <button
            onClick={onClose}
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground truncate">
                  {enableNotifs ? '可在此随时调整提醒时间' : '关闭后跳过时间设置'}
                </p>
              </div>
            </div>
